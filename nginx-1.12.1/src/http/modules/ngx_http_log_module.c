...
static ngx_http_log_var_t  ngx_http_log_vars[] = {
    ...
    { ngx_string("request_time"), (NGX_TIME_T_LEN + 4) * 3,
...
{
//    ngx_time_t      *tp;
    ngx_msec_int_t   us; //Yu: Support for microsecond percision
    ngx_msec_int_t   start_us; //Yu: Support for microsecond percision
    ngx_msec_int_t   end_us; //Yu: Support for microsecond percision
...
//    tp = ngx_timeofday();
    ...
    us = (ngx_msec_int_t)
             ((tv.tv_sec - r->start_sec) * 1000000 + (tv.tv_usec - r->start_usec));
    us = ngx_max(us, 0);

    start_us = r->start_sec * 1000000 + r->start_usec;
    end_us = tv.tv_sec * 1000000 + tv.tv_usec;

    return ngx_sprintf(buf, "%T.%06M, %T, %T", (time_t) us / 1000000, us % 1000000, start_us, end_us);
}
